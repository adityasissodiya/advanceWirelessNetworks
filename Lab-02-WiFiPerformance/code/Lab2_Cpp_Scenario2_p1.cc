...
main (int argc, char *argv[])
{
  ...
  double   distance = 1.0;   // Distance between Nodes
  double   antHeight = 1.5;   // Antenna height
  ...
  const double h = distance * std::sqrt(3.0) / 2.0;
  pos->Add(Vector( 0.0, 0.0, antHeight));     // AP
  pos->Add(Vector(-distance/2.0, h, antHeight));  // STA sender
  pos->Add(Vector(distance/2.0, h, antHeight));  // STA receiver
  ...
  anim.SetConstantPosition(staNodes.Get(0), -distance/2, h);
  anim.SetConstantPosition(staNodes.Get(1), distance/2, h);
  ...
  std::cout << "[Scenario2 p1] PHYMode=" << mode
            << "  distance=" <<distance 
            << "  antHeight=" <<antHeight
            << "  payload=" <<payload
            << "  offered=100Mbps"
